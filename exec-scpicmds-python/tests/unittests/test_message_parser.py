...
class TestMessageParser(unittest.TestCase):
    ...
    def test_get_command_type_from_string(self) -> None:
        self.assertEqual(MessageParser.get_command_type_from_string("TEST?"), CommandType.QUERY, "Wrong command type returned.")
        self.assertEqual(MessageParser.get_command_type_from_string("TEST"), CommandType.COMMAND, "Wrong command type returned.")
        self.assertEqual(MessageParser.get_command_type_from_string("TEST myparam;"), CommandType.COMMAND, "Wrong command type returned.")
        self.assertEqual(MessageParser.get_command_type_from_string("  "), CommandType.EMPTY, "Wrong command type returned.")

    def test_check_message_is_valid(self) -> None:
        self.assertCountEqual(CommandType, [CommandType.UNKNOWN, CommandType.COMMAND, CommandType.QUERY, CommandType.EMPTY], "Not all enum values are tested.")
        for command_types in [[CommandType.COMMAND], [CommandType.QUERY], [CommandType.COMMAND, CommandType.QUERY]]:
            ...
            self.assertTrue(MessageParser.check_message_is_valid(message_data), "Validity-check of message data incorrectly returned a negative result.")
        for command_types in [[], [CommandType.EMPTY], [CommandType.UNKNOWN], [CommandType.COMMAND, CommandType.EMPTY], [CommandType.QUERY, CommandType.UNKNOWN]]:
            message_data = MessageData()
            message_data.commands=[CommandData("", "", command_type) for command_type in command_types]
            self.assertFalse(MessageParser.check_message_is_valid(message_data), "Validity-check of message data incorrectly returned a negative result.")
    ...
    def test_get_message_data_from_string(self) -> None:
        ...
        self.assertFalse(message_data.is_valid, "Message data: is_valid-property parsed incorrectly.")
        self.assertEqual(message_data.file_line_number, line_number, "Message data: file_line_number-property parsed incorrectly.")
        self.assertEqual(message_data.original_message, message, "Message data: original_message-property parsed incorrectly.")
        self.assertEqual(message_data.command_count_string_width, len(str(len(message_data.commands))), "Message data: command_count_string_width-property parsed incorrectly.")
        ...
        for result_idx, result in enumerate(expected_results):
            self.assertEqual(message_data.commands[result_idx].command, result[0], f"Command data {result_idx}: command-property parsed incorrectly.")
